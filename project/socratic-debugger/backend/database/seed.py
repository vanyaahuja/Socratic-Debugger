...
def seed():
    ...
    db = SessionLocal()
    try:
        # The frontend hardcodes USER_ID = 1 (no auth/signup flow yet in
        # this scaffold) -- without this row, every /sessions call fails
        # its foreign-key check with an unhandled IntegrityError.
        if db.query(User).filter(User.id == 1).first() is None:
            db.add(User(id=1, name="Guest"))
            db.commit()
